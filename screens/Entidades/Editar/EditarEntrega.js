...
export default function EditarentregaScreen({ navigation, route }) {
  ...
  async function carregarDados() {
    try {
      ...
      // Carrega os dados da entrega
      let entregaData;
      if (useLocalData) {
        entregaData = await databaseService.selectById('entrega', entregaId);
      } else {
      ...
      if (entregaData) {
        setFormData({
          ...entregaData,
          data_saida: new Date(entregaData.data_saida),
          quantidade: String(entregaData.quantidade),
          valor_unitario: String(entregaData.valor_unitario),
          valor_total: String(entregaData.valor_total),
          nota: entregaData.nota || 0, // Garante que tenha valor padrão
        });
      }
    } catch (error) {
  ...
  async function salvarAlteracoes() {
    ...
    try {
      ...
      if (networkState.isConnected) {
        ...
        await databaseService.update('entrega', entregaData, 'id = ?', [entregaId]);
      }
  ...
  if (loading) {
    return (
      <View style={styles.container}>
        ...
        <Text style={styles.emptyText}>Carregando dados da entrega...</Text>
      </View>
    );
  }
    ...
    >
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>EDITAR ENTREGA</Text>
