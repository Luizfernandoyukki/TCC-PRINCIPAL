...
export const initDatabase = async () => {
  return new Promise(async (resolve, reject) => {
    if (Platform.OS === 'web') {
      ...
      reject(new Error('SQLite não disponível no navegador.'));
      return;
    }
    ...
    try {
      const session = new SQLite.SQLiteSession(db);
...
// ATENÇÃO: 
// - Troque todos os tx.executeSql(...) por session.runSync(...)
// - Use session.runSync para cada comando SQL de criação de tabela/trigger
// - Feche a sessão com session.closeSync() no final